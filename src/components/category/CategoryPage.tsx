      ...
      {showAddForm && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">父分类</label>
              <select
                value={newParentId || ''}
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-400">({children.length} 个子分类)</span>
                </div>
                <div className="flex items-center gap-1">
                  <button
                    onClick={(e) => {
...
function AddSubCategory({ parentId, onAdded }: {
  ...
  if (!showForm) {
    return (
      <button
        ...
        className="w-full px-4 py-3 text-sm text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors flex items-center justify-center gap-1 border-t border-slate-200"
      >
  ...
  return (
    <div className="px-4 py-3 flex items-center gap-2 border-t border-slate-200">
      <input
        type="text"
        value={name}
        onChange={(e) => setName(e.target.value)}
        placeholder="子分类名称"
        className="flex-1 px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
        autoFocus
        onKeyDown={(e) => e.key === 'Enter' && handleAdd()}
      />
      <button onClick={handleAdd} className="p-1.5 text-green-600 hover:bg-green-50 rounded transition-colors">
        <Check className="w-4 h-4" />
      </button>
      <button onClick={() => setShowForm(false)} className="p-1.5 text-slate-400 hover:bg-slate-200 rounded transition-colors">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
